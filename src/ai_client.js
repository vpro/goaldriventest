...
class AIClient {
    ...
    getPromptHistoryTextOnly() {
        return this.#promptHistory.map( ( prompt ) => ( {
            role: prompt.role,
            content: prompt.content.filter( ( content ) => content.type === 'text' ).map( ( content ) => content )
        } ) );
    }
...
class AIPlaybackClient extends AIClient {
    ...
    #readPrompts( filename ) {
        ...
        const messages = allmessages.filter( ( promptMessage ) => promptMessage.role === 'assistant' );
